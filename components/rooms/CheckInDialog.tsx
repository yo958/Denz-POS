...
interface CheckInDialogProps {
  ...
  room: Product | null;
  onClose: () => void;
  onCheckIn: (data: { guestName: string; guestPhone?: string; nights: number; notes?: string }) => void;
}
...
export function CheckInDialog({ open, room, onClose, onCheckIn }: CheckInDialogProps) {
  ...
  useEffect(() => {
    if (open) { setName(''); setPhone(''); setNights(1); setNotes(''); }
  }, [open]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Check in guest">
      ...
      <form
        onSubmit={e => { e.preventDefault(); if (canSubmit) onCheckIn({ guestName: name.trim(), guestPhone: phone.trim() || undefined, nights, notes: notes.trim() || undefined }); }}
