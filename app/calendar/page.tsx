...
import { useCurrentStaff, useSpaces, useStays, useProducts } from '@/lib/hooks/useStore';
...
function AcceptedPill({ name }: { name: string }) {
  return (
    <span className="block w-full rounded px-1.5 py-0.5 text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 truncate">
      {shortName(name)}
    </span>
  );
}
function PendingPill({ name }: { name: string }) {
  return (
    <span className="block w-full rounded px-1.5 py-0.5 text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300 truncate">
      {shortName(name)}
    </span>
  );
}
function StayPill({ name }: { name: string }) {
  return (
    <span className="block w-full rounded px-1.5 py-0.5 text-xs font-medium bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 truncate">
      {shortName(name)}
    </span>
  );
}
...
export default function CalendarPage() {
  ...
  // Firestore: accepted + pending coworking/room-enquiry orders with a bookingDate
  useEffect(() => {
  ...
  // ─── Grid helpers ──
  function coworkCellBookings(spaceId: string, spaceName: string, day: Date) {
  ...
  function roomCellBookings(roomId: string, roomName: string, day: Date) {
  ...
  function allBookingsForDay(day: Date) {
    const cowork = webOrders.filter(o => o.type === 'coworking' && webOrderCoversDay(o, day));
    ...
    return { cowork, roomOrders, roomStays };
  }
      ...
      <div className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b border-border px-4 py-3 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center rounded-xl border border-border overflow-hidden text-xs font-medium">
            <button
              ...
              className={`px-3 py-1.5 transition-colors ${view === 'grid' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}
            >
              Grid
            </button>
            <button
              onClick={() => setView('list')}
              className={`px-3 py-1.5 transition-colors ${view === 'list' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}
            >
              List
            </button>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 h-8 rounded-lg border border-border text-xs font-medium hover:bg-muted transition-colors"
          >
            Today
          </button>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-green-400 inline-block" />Accepted</span>
      ...
      {view === 'grid' ? (
            ...
            <thead>
              <tr>
                ...
                {weekDays.map((d, i) => (
                  <th
                    key={i}
                    className={`border-b border-r border-border px-2 py-2 text-center text-xs font-semibold ${isToday(d) ? 'bg-primary/10 text-primary' : 'bg-muted/40 text-muted-foreground'}`}
                  >
                    <div>{DAY_NAMES[i]}</div>
            ...
            <tbody>
                ...
                <>
                  <tr>
                    <td colSpan={8} className="bg-muted/60 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border">
                      <Monitor size={11} className="inline mr-1.5 -mt-0.5" />
                      Coworking Spaces
                    </td>
                  </tr>
                  {spaces.map(space => (
                    <tr key={space.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                      <td className="border-r border-border px-3 py-2 text-xs font-medium text-foreground truncate max-w-[140px]">
                        {space.name}
                      </td>
                      {weekDays.map((day, i) => {
                        const bookings = coworkCellBookings(space.id, space.name, day);
                        return (
                          <td
                            key={i}
                            className={`border-r border-border px-1 py-1 align-top min-w-[80px] ${isToday(day) ? 'bg-primary/5' : ''}`}
                          >
              ...
              {rooms.length > 0 && (
                <>
                  <tr>
                    <td colSpan={8} className="bg-muted/60 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border">
                      <BedDouble size={11} className="inline mr-1.5 -mt-0.5" />
                      Rooms
                    </td>
                  </tr>
                  {rooms.map(room => (
                    <tr key={room.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                      ...
                      {weekDays.map((day, i) => {
                        const { fromStays, fromOrders } = roomCellBookings(room.id, room.name, day);
                        return (
                          <td
                            key={i}
                            className={`border-r border-border px-1 py-1 align-top min-w-[80px] ${isToday(day) ? 'bg-primary/5' : ''}`}
              ...
              {spaces.length === 0 && rooms.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-16 text-center text-sm text-muted-foreground">
                    No spaces or rooms configured yet.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-border">
          {weekDays.map((day, idx) => {
            const { cowork, roomOrders, roomStays } = allBookingsForDay(day);
            const total = cowork.length + roomOrders.length + roomStays.length;
            return (
                ...
                <div className="flex items-center gap-3 px-4 pt-4 pb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {day.toLocaleDateString('en-GB', { weekday: 'long', month: 'long', year: 'numeric' })}
                    </p>
                  </div>
                  {total > 0 && (
                ...
                {total === 0 ? (
                  ...
                  <div className="px-4 pb-4 space-y-2">
                    ...
                    {cowork.map(o => {
                      ...
                      const startDate = o.bookingDate
                        ? new Date(o.bookingDate + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })
                        : null;
                      return (
                        <div key={o.id} className="flex items-start gap-3 rounded-xl border border-border bg-card p-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground truncate">
                              {space?.name ?? o.tableOrSpace ?? 'Coworking'}
